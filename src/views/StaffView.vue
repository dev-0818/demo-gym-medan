...
const { isAdmin, canManageStaff } = usePermissions()
const search = ref('')
...
function save() {
  ...
  if (editingUser.value) {
    ...
    logStore.addLog({ action: 'update', targetType: 'staff', targetId: editingUser.value.id, targetName: form.name, details: `Mengupdate data staff "${form.name}"` })
  } else {
    const result = userStore.addUser({ ...form, photo: photoData, avatar: photoData, role: 'staff', isActive: true })
    generatedPassword.value = result.password
    generatedUserName.value = result.user.name
    showPasswordModal.value = true
    logStore.addLog({ action: 'create', targetType: 'staff', targetId: result.user.id, targetName: result.user.name, details: `Membuat akun staff baru "${result.user.name}"` })
  }
  showModal.value = false
}

function resetPassword(user: User) {
  resetConfirmMessage.value = `Yakin ingin reset password staff "${user.name}"? Password lama akan diganti dengan password baru.`
...
function confirmResetPassword() {
  ...
  showPasswordModal.value = true
  logStore.addLog({ action: 'reset_password', targetType: 'staff', targetId: user.id, targetName: user.name, details: `Reset password staff "${user.name}"` })
  pendingResetUser.value = null
}

function deleteUser(user: User) {
  confirmMessage.value = `Yakin hapus staff "${user.name}"? Data yang sudah dihapus tidak bisa dikembalikan.`
...
function confirmDelete() {
  ...
  if (user) {
    logStore.addLog({ action: 'delete', targetType: 'staff', targetId: pendingDeleteId.value, targetName: user.name, details: `Menghapus staff "${user.name}"` })
  }
}
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Kelola Staff</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400">Generate akun & password untuk staff</p>
      </div>
      <button v-if="canManageStaff" @click="openAdd" class="btn-primary">
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 text-left">
              <th class="px-5 py-3.5 font-semibold text-gray-600 dark:text-gray-400">Staff</th>
            ...
            <tr v-for="s in filteredStaff" :key="s.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" @click="openDetail(s)">
              <td class="px-5 py-3.5">
                <div class="flex items-center gap-3">
                  ...
                  <div v-else class="flex h-10 w-10 items-center justify-center rounded-full bg-blue-100 text-blue-700 text-sm font-semibold">{{ s.name.charAt(0) }}</div>
                  <div>
              ...
              <td class="px-5 py-3.5 text-gray-600 dark:text-gray-400 font-mono text-xs">{{ s.nik || '-' }}</td>
              <td class="px-5 py-3.5 text-gray-600 dark:text-gray-400">{{ s.phone }}</td>
              <td class="px-5 py-3.5 text-gray-600 dark:text-gray-400">
                <span v-if="s.birthDate">{{ calculateAge(s.birthDate) }} th</span>
                <span v-else>-</span>
              </td>
              <td class="px-5 py-3.5" @click.stop>
                <div class="flex items-center gap-1">
                  <code class="text-xs font-mono text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">{{ visiblePasswords.has(s.id) ? s.password : maskPassword(s.password) }}</code>
                  <button @click="togglePasswordVisibility(s.id)" class="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors" :title="visiblePasswords.has(s.id) ? 'Sembunyikan' : 'Lihat password'">
                    <svg v-if="visiblePasswords.has(s.id)" xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.879L21 21" /></svg>
                    <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  </button>
                </div>
              </td>
              <td class="px-5 py-3.5" @click.stop>
                <button v-if="canManageStaff" @click="userStore.toggleActive(s.id)" :class="['inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium cursor-pointer transition-colors', s.isActive ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-red-100 text-red-700 hover:bg-red-200']">
                  {{ s.isActive ? 'Aktif' : 'Nonaktif' }}
                </button>
                <span v-else :class="['inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium', s.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700']">
                  {{ s.isActive ? 'Aktif' : 'Nonaktif' }}
                </span>
              </td>
              <td class="px-5 py-3.5" @click.stop>
                <div v-if="canManageStaff" class="flex items-center justify-end gap-1">
                  <button @click="resetPassword(s)" class="btn-ghost btn-xs" title="Reset Password">
                  ...
                  <button @click="openEdit(s)" class="btn-ghost btn-xs" title="Edit">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  </button>
                  <button @click="deleteUser(s)" class="btn-ghost btn-xs text-red-500 hover:text-red-700" title="Hapus">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                  </button>
                </div>
                <span v-else class="text-xs text-gray-400 dark:text-gray-500 flex justify-end">—</span>
              </td>
            </tr>
            <tr v-if="filteredStaff.length === 0"><td colspan="7" class="px-5 py-12 text-center text-gray-500 dark:text-gray-400">Tidak ada staff ditemukan.</td></tr>
          </tbody>
        </table>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <ModalDialog v-model="showModal" :title="editingUser ? 'Edit Staff' : 'Tambah Staff Baru'" size="lg">
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="sm:col-span-2">
              ...
              <textarea v-model="form.notes" class="input" rows="2" placeholder="Catatan khusus, kondisi medis, dll."></textarea>
            </div>
          </div>
        </div>
    ...
    <!-- Detail Modal -->
    <ModalDialog v-model="showDetailModal" title="Detail Staff" size="lg">
      <div v-if="detailUser" class="space-y-5">
        <div class="flex items-center gap-5">
          <img v-if="detailUser.photo || detailUser.avatar" :src="detailUser.photo || detailUser.avatar" :alt="detailUser.name" class="h-20 w-20 rounded-full object-cover ring-4 ring-blue-100 dark:ring-blue-900/30" />
          <div v-else class="flex h-20 w-20 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-2xl font-bold">{{ detailUser.name.charAt(0) }}</div>
          <div>
      ...
      <template #footer>
        <div class="flex justify-between">
          <button v-if="detailUser && canManageStaff" @click="showDetailModal = false; openEdit(detailUser)" class="btn-secondary">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            Edit
          </button>
    ...
    <ModalDialog v-model="showPasswordModal" title="🔑 Password Staff" size="sm">
